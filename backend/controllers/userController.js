...
const getUserPreferences = async (req, res, next) => {
    try {
        ...
        if (prefs.length === 0) {
            // This can happen for users created before the preferences table existed
        ...
        res.json({
            ...prefs[0],
            contentPreferences: prefs[0].content_preferences, // Ensure correct key casing
...
// @route   POST /api/users/subscribe-newsletter
// @access  Public
const subscribeToNewsletter = async (req, res, next) => {
    ...
    try {
        ...
        if (authHeader && authHeader.startsWith('Bearer')) {
            ...
            if(decoded.id) {
                await db.query('UPDATE user_preferences SET newsletter_subscribed = 1 WHERE user_id = ?', [decoded.id]);
            }
        }
...
const getNotifications = async (req, res, next) => {
    try {
        const [notifications] = await db.query(`
            SELECT 
                n.id, n.type, n.actor_name as actorName, n.is_read as isRead, n.createdAt,
                n.related_article_id as relatedArticleId,
                a.title as relatedArticleTitle
...
// @route POST /api/users/me/notifications/mark-as-read
// @access Protected
const markNotificationsAsRead = async (req, res, next) => {
    const { ids } = req.body; // ids can be an array of notification IDs
    if (!ids || !Array.isArray(ids) || ids.length === 0) {
        return res.status(400).json({ message: 'Notification IDs array is required.' });
    }
    try {
        const query = 'UPDATE notifications SET is_read = 1 WHERE user_id = ? AND id IN (?)';
        await db.query(query, [req.user.id, ids]);
        res.status(204).send();
    } catch (error) {
        next(error);
    }
